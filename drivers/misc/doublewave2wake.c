...
#define DRIVER_DESCRIPTION "Doublewave2wake for taoshan"
...
static void doublewave2wake_presspwr(struct work_struct * doublewave2wake_presspwr_work) {
	...
	pm8xxx_vib_enable_ms(DW2W_VIBRATOR_WAKE);
	return;
}
...
static void new_wave(void) {
	...
	wave_nr++;
	pm8xxx_vib_enable_ms(DW2W_VIBRATOR_WAVE);
}
...
#endif
	if (strstr(dev->name, "tsl2772_proximity") || strstr(dev->name, "pmic8xxx_pwrkey")) {
		return 0;
	} else {
		return 1;
	}
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void dw2w_early_suspend(struct early_suspend *h) {

	dw2w_scr_suspended = true;
}

static void dw2w_late_resume(struct early_suspend *h) {
...
static struct early_suspend dw2w_early_suspend_handler = {
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	register_early_suspend(&dw2w_early_suspend_handler);
#endif // CONFIG_HAS_EARLYSUSPEND
